...
1) Установите mkcert (Windows):
   ...
   mkcert -CAROOT
   ```
2) Сгенерируйте сертификат прямо в папку `certs` для домена/адреса, по которому будете открывать с iPhone:
   ```powershell
   mkdir certs 2>$null
   # Пример: IP и localhost (добавьте свой домен, если используете mDNS/DNS)
   mkcert -cert-file certs/server.crt -key-file certs/server.key 192.168.88.156 localhost
   ```
...
- Убедитесь, что открыт 443 и включён редирект 80 → 443 (в конфиге уже настроено).

### Временный доступ по HTTP
- Для быстрой проверки UI можно открыть приложение по `http://<host>:3000` (без HTTPS). В этом режиме PWA‑функции, требующие HTTPS (например, Push), работать не будут. Чтобы вернуть принудительный HTTPS, оставьте редирект с 80 на 443 в `nginx.conf`.
